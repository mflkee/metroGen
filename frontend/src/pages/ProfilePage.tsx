...
import { passwordPolicyMessage, isPasswordPolicyValid } from "@/lib/password";
...
export function ProfilePage() {
  ...
  return (
    <section>
      <PageHeader title="Профиль" description="Учетная страница с базовой информацией и сменой пароля." />
        ...
        {user ? (
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,420px)]">
            ...
            <div className="space-y-4">
              <form className="tone-child space-y-4 rounded-2xl border border-line p-4" onSubmit={handleSaveProfile}>
                <div>
                  <h3 className="text-base font-semibold text-ink">Данные профиля</h3>
                  <p className="mt-1 text-sm text-steel">
                    Эти поля не участвуют в генерации напрямую, но нужны для полноценного пользовательского контура metroGen.
                  </p>
                </div>

                <label className="block text-sm text-steel">
                  Телефон
                  <input className="form-input" type="text" value={phone} onChange={(event) => setPhone(event.target.value)} />
                </label>
                <label className="block text-sm text-steel">
                  Организация
                  <input className="form-input" type="text" value={organization} onChange={(event) => setOrganization(event.target.value)} />
                </label>
                <label className="block text-sm text-steel">
                  Должность
                  <input className="form-input" type="text" value={position} onChange={(event) => setPosition(event.target.value)} />
                </label>
                <label className="block text-sm text-steel">
                  Объект
                  <input className="form-input" type="text" value={facility} onChange={(event) => setFacility(event.target.value)} />
                </label>
              ...
              <form className="tone-child space-y-4 rounded-2xl border border-line p-4" onSubmit={handleChangePassword}>
                <div>
                ...
                <PasswordInput autoComplete="current-password" label="Текущий пароль" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} />
                <PasswordInput autoComplete="new-password" label="Новый пароль" minLength={6} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} />
                <PasswordInput autoComplete="new-password" label="Подтверждение нового пароля" minLength={6} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} />
          ...
          <p className="text-sm text-steel">Данные профиля загружаются после проверки текущей сессии.</p>
        )}
      </div>
    </section>
  );
