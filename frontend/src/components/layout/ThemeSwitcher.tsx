...
export function ThemeSwitcher() {
  ...
  async function handleSelect(value: string) {
    ...
    try {
      ...
      // local theme is already applied; next profile fetch will resync if needed
    }
  }
    ...
    <div ref={pickerRef} className="theme-picker">
      ...
      <div className="theme-menu">
        ...
        >
          ...
          <span className={["theme-menu__caret", isOpen ? "theme-menu__caret--open" : ""].join(" ")}>▾</span>
        </button>
        {isOpen ? (
            ...
            {options.map((option) => (
              ...
              >
                <span className="theme-menu__option-main">
                  ...
                  {option.preview ? <span className="theme-menu__preview">{option.preview}</span> : null}
                </span>
                {selectedValue === option.value ? <span className="theme-menu__status">Текущая</span> : null}
              </button>
            ))}
          </div>
        ) : null}
