...
- **Границы:** агент отвечает за полностью подготовленный контекст (без итогового рендера). Сторонние зависимости: HTTP клиент, `AsyncSession`.
- **Ошибки:** `ProtocolContextItem.error` фиксирует проблемы («certificate number is empty», `http error: ...`). Роутеры агрегируют ошибки и продолжают работу над остальными строками.
...
- **Поведение:**  
  - `ensure_owner`/`ensure_methodology` создают записи, попутно вшивая алиасы/точки методик из `data/*.json`.  
...
- **Особенности:**  
  ...
  - `normalize_serial`/`normalize_owner_alias` обеспечивают одинаковые ключи в базе.
