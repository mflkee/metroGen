...
### Уже сделано
- Переработана страница `frontend/src/pages/GenerationPage.tsx`:
  - добавлены рабочие зоны `Генерация`, `База данных`, `Готовые генерации`;
  - добавлен визуальный прогресс по этапам batch-генерации;
  - добавлен импорт реестра из UI;
  - добавлена пакетная выдача run-папки через ZIP;
  - история готовых run-папок вынесена в отдельную рабочую зону.
- Расширен backend для UI:
  - `GET /api/v1/system/export-folder-archive`;
  - `GET /api/v1/resolve/status`;
  - `system/status` теперь возвращает срез по БД и экспортам.
- Приведены к более зрелому виду админские страницы:
  - `frontend/src/pages/AdminUsersPage.tsx`;
  - `frontend/src/pages/DeveloperDashboardPage.tsx`.
- Добавлен branding в стиле `metroLog`:
  - favicon `g`;
  - `BrandMark` в auth/shell;
  - topbar-индикатор доступности Аршина.
- Создана учетная запись `makeevgb@mkair.ru` в рабочей БД `metroGen` с ролью `DEVELOPER`.

### Следующий продуктовый приоритет
- Довести `frontend/src/pages/UserDetailsPage.tsx` до паритета с обновлённой страницей пользователей:
  - вынести удаление в confirm-modal;
  - добавить историю логинов/активности;
  - добавить copy/reset-поток для временного пароля без возврата в список.
- Пересобрать `frontend/src/pages/ArshinPage.tsx` под тот же уровень UX:
  - понятные статусы запросов;
  - история последних проверок;
  - подготовка к массовой валидации сертификатов из Excel.
- Укрепить `frontend/src/pages/DashboardPage.tsx`:
  - быстрые CTA в конкретные workspace-секции страницы генерации;
  - отдельный блок по состоянию БД;
  - последняя активность пользователей для admin/developer.

### Генерация и выдача
- Добавить серверный контракт run-summary:
  - длительность по этапам;
  - количество строк источника;
  - количество успешных/неуспешных контекстов;
  - метки retry по batch.
- Добавить долгоживущий журнал генераций:
  - отдельная таблица `generation_runs`;
  - хранение `run_id`, статуса, параметров, итоговой папки, ошибок;
  - UI-история не только по файловой системе, но и по сущностям.
- Добавить потоковый прогресс:
  - WebSocket/SSE или poll-эндпоинт;
  - реальный, а не визуально-эвристический progress bar;
  - вывод этапов `upload -> build context -> retry -> render -> save`.
- Добавить экспорт всего run не только в ZIP, но и в:
  - единый PDF merge;
  - CSV/JSON отчёт по ошибкам;
  - manifest c метаданными run.

### База данных и справочники
- Сделать полноценную вкладку БД, а не только импорт:
  - просмотр активных строк реестра;
  - фильтры по `instrument_kind`, источнику, месяцу, серийнику;
  - просмотр последнего `loaded_at` и источника строки.
- Добавить CRUD по вспомогательным средствам поверки в UI.
- Добавить UI для owners/methodologies:
  - поиск;
  - просмотр алиасов;
  - редактирование ИНН владельца;
  - редактирование пунктов методики и допустимой вариации.
- Зафиксировать импорт-диагностику:
  - сколько строк пропущено;
  - какие заголовки не совпали;
  - какие записи деактивированы при reimport.

### Пользователи и мониторинг
- Подтянуть максимальный паритет с `metroLog`:
  - единые паттерны таблиц, карточек и служебных метрик;
  - одинаковые статусы online/offline;
  - одинаковый copy/reset flow по временным паролям.
- Добавить служебные метрики в мониторинг:
  - среднее время генерации;
  - количество ошибок Аршина за период;
  - количество failed протоколов;
  - health по Playwright/Chromium и SMTP.
...
### Технический долг
- Разбить `app/api/routes/protocols.py` на модули по сценариям генерации.
- Вынести общую структуру ответа PDF-ручек в typed schema.
- Перевести system/export архив на переиспользуемый service-layer helper.
- Добавить frontend-tests на новые рабочие зоны generation/admin/developer.
- Зафиксировать e2e-проверку:
  - login;
  - import registry;
  - generate pdf;
  - download zip;
  - create/reset/delete user.
